...
\inred{IT MAY BE DEDUCED IN AN APPENDIX OR SUPPLEMENTARY MATERIAL FOLLOWING LSBL1440}
...
The mirror height is $z$=0 at the mirror center position $x, y$ = 0, as expected. The tangential profile ($x$=0) is a parabola $y=q \sin2\theta + z^2/(4f)$ and focal length $ f = q (1-\cos2\theta) /2$. For small angles ($\cos2\theta\approx 1 - 2\theta^2$) and $|y|\ll q$ gives $z\approx 2 \theta q + \theta y$ with slope in the center $dz/dy=\theta$. The sagittal section ($y$=0) is an ellipse $x^2/a^2 + (z + q \sin2\theta)^2/b^2=1$ with semiaxes $a=q \sin2\theta$ and $b=a \sqrt{p /(p+q)}$ \inred{REF HOWARD LSBL 1465}. 
...
%For practical purposes, both numerical implementations of the diaboloid are good enough. 
...
%The linearized parabolic cone possibly is useful as it gives a way to make a simpler surface that might be easier to manufacture.
