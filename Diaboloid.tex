...
Because a parabolic surface can provide vertical collimation of a point source in one direction, the central vertical cross-section (i.e. the tangential shape) of the mirror will be parabolic. \inred{The sagittal curvature provide point-to-point focusing for rays outside of this central plane of incidence.}
...
% Ken thinks we should have a section break here.
...
% Seems that this could be a sub-section break?
...
The diaboloid is implemented in an approximate way (using the Eq.\ref{eqn:diaboloidV} affected by a linear detrending $y\theta$ to account for the axes rotation) or in an exact form by solving numerically its implicit quartic equation \cite{val2021} using the {\tt fqs} python library by N. Krvavika ({\tt https://github.com/NKrvavica/fqs}). 
...
The deposition of a thin correcting layer at the edge of the cylindrical substrate seems to be an attractive way to make diaboloids, at low glancing angles. % at least for the \SI{5}{\milli\radian} grazing angle case.
...
\inred{We acknowledge Wayne McKinney for valuable discussions and insight.}
